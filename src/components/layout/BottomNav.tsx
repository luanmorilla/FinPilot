          ...
          {navItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className="relative -mt-8">
                  <motion.div whileTap={{ scale: 0.92 }} className="relative flex flex-col items-center">
                    ...
                    <span className={`text-[10px] font-medium mt-1 ${isActive ? "text-purple-400" : "text-zinc-500"}`}>
                      {item.label}
                    </span>
                  </motion.div>
                </Link>
            ...
            return (
              <Link key={item.href} href={item.href}>
                <motion.div whileTap={{ scale: 0.88 }} className="flex flex-col items-center gap-1 px-3 py-1 min-w-[52px]">
                  <div className={`relative w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 ${isActive ? "bg-emerald-500/15" : "bg-transparent"}`}>
                    {isActive && (
                      <motion.div
                        layoutId="nav-active"
                    ...
                    {Icon && (
                      <Icon
                        size={20}
                        className={`relative z-10 transition-colors duration-200 ${isActive ? "text-emerald-400" : "text-zinc-500"}`}
